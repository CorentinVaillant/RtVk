...
Mesh::Mesh(fastgltf::Asset &asset, fastgltf::Mesh &mesh,
  ...
  for (fastgltf::Primitive &primitive : mesh.primitives) {
    ...
    fastgltf::iterateAccessorWithIndex<uint32_t>(
        asset, accessor,
        [vertex_offset, &indices, vertex_buffer, this](uint32_t index,
                                                       size_t i) {
          indices[i % 3] = index + vertex_offset;

          if (i % 3 == 2) {
            _indices.emplace_back(indices[0], indices[1], indices[2]);
            bbox = BBox(bbox, _indices.back().get_bbox(vertex_buffer.data()));
          }
        });
...
bool Mesh::hit(Ray r, Interval ray_t, HitRecord *records, std::span<const Vertex> vertex_buffer) const {
  bool hit_anything = false;

  for (const TriangleIndices &triangle : _indices) {
    if (triangle.hit(vertex_buffer.data(), r, ray_t, records)) {
      ray_t.max = records->t;
      hit_anything = true;
    }
  }
